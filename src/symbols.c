...
extern void as_bad_where (const char *file, unsigned int line, const char *format, ...);
struct symbol_flags
{
...
S_FORCE_RELOC (symbolS *s, int strict) {
  ...
  if (s->flags.local_symbol)
    ...
    {
        // printf("strict:%d\n",strict);
        // printf("s->bsym->flags:%d\n",s->bsym->flags);
        // printf("BSF_WEAK:%d\n",BSF_WEAK);
        // printf("EXTERN_FORCE_RELOC:%d\n",EXTERN_FORCE_RELOC);
        // printf("BSF_GLOBAL:%d\n",BSF_GLOBAL);
...
/**
 * @Description: 将符号的值设置为当前代码片段中的当前位置
 */
void
